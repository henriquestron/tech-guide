...
// Edite aqui para adicionar mais opções conforme seu banco de dados
const subcategoryOptions: Record<string, { label: string; slug: string }[]> = {
  ...
  // Adicione 'beleza', 'notebooks', etc...
};

interface FilterProps {
  onFilterChange: (filters: FilterState) => void;
  brands: string[];
  categorySlug: string; // <--- NOVO: Precisamos saber em qual categoria estamos
}
...
export default function FilterSidebar({ onFilterChange, brands, categorySlug }: FilterProps) {
  ...
  // Pega as opções baseadas na categoria atual (ex: pecas)
  ...
  // Função para navegar entre subcategorias
  const handleSubcategoryClick = (slug: string) => {
    const params = new URLSearchParams(searchParams.toString());
    
    if (currentSub === slug) {
      params.delete("sub"); // Se clicar no mesmo, remove o filtro
    } else {
      params.set("sub", slug);
    }
  ...
  const clearSubcategory = () => {
    router.push(pathname); // Volta para a categoria raiz
  };
